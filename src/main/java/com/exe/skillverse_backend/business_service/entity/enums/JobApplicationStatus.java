...
public enum JobApplicationStatus {
    ...
    OFFER_SENT, // Offer letter sent to candidate (REMOTE job, is_negotiable=true only)
    ...
    AWAITING_ONBOARDING_INFO, // Waiting for candidate to submit CCCD + bank info (REMOTE job only)
    HIRED, // Candidate hired — final status for ONSITE jobs (offline contract process)
    REJECTED, // Application rejected by recruiter (with reason)
    CONTRACT_SIGNED  // Contract has been signed by both parties (REMOTE job only)
}

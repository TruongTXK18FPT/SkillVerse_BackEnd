...
public class CVGeneratorAIServiceImpl implements CVGeneratorAIService {
                ...
                for (int attempt = 1; attempt <= maxAttempts; attempt++) {
                    try {
                        ...
                        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                            ...
                            log.info("CV generated successfully for user: {} with model {}", profile.getUserId(),
                                    currentModel);
                            return cvContent;
                        }

                        log.warn("Mistral response not OK ({}), attempt {}/{} with model {}", response.getStatusCode(),
                                attempt, maxAttempts, currentModel);
                    } catch (HttpClientErrorException e) {
                        if (e.getStatusCode() == HttpStatus.TOO_MANY_REQUESTS) {
                            long backoffMs = (long) (1000L * Math.pow(2, attempt - 1));
                            log.warn("429 from Mistral (model {}), backing off {} ms, attempt {}/{}", currentModel,
    ...
    private String buildCVPrompt(
        ...
        prompt.append("\n\n🎯 FINAL REQUIREMENTS:\n");
        prompt.append("\n1. Follow the EXACT color scheme specified for the ")
                .append(request.getTemplateName() != null ? request.getTemplateName().toUpperCase() : "PROFESSIONAL")
                .append(" template");

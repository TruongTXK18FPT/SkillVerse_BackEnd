...
public class LogisticsPromptServiceImpl extends BaseExpertPromptService {
    ...
    public String getPrompt(String domain, String industry, String role) {
        ...
        boolean isLogisticsOps = normalizedIndustry.contains("logistics operations")
                || normalizedIndustry.contains("vận hành logistics") ||
        ...
        if (isLogisticsOps) {
            if (normalizedRole.contains("logistics coordinator") || normalizedRole.contains("điều phối logistics"))
                return getLogisticsCoordinatorPrompt();
            if (normalizedRole.contains("warehouse staff") || normalizedRole.contains("nhân viên kho"))
                return getWarehouseStaffPrompt();
            if (normalizedRole.contains("warehouse manager") || normalizedRole.contains("quản lý kho"))
                return getWarehouseManagerPrompt();
            if (normalizedRole.contains("inventory controller") || normalizedRole.contains("kiểm soát tồn kho"))
                return getInventoryControllerPrompt();
            if (normalizedRole.contains("fulfillment specialist")
                    || normalizedRole.contains("chuyên viên hoàn thành đơn"))
                return getFulfillmentSpecialistPrompt();
            if (normalizedRole.contains("supply chain planner")
                    || normalizedRole.contains("nhà hoạch định chuỗi cung ứng"))
                return getSupplyChainPlannerPrompt();
            if (normalizedRole.contains("transport planner") || normalizedRole.contains("nhà hoạch định vận tải"))
                return getTransportPlannerPrompt();
            if (normalizedRole.contains("fleet manager") || normalizedRole.contains("quản lý đội xe"))
                return getFleetManagerPrompt();
            if (normalizedRole.contains("distribution center operator")
                    || normalizedRole.contains("vận hành trung tâm phân phối"))
                return getDistributionCenterOperatorPrompt();
        }
        ...
        if (isFreightShipping) {
            if (normalizedRole.contains("freight forwarder") || normalizedRole.contains("giao nhận vận tải quốc tế"))
                return getFreightForwarderPrompt();
            if (normalizedRole.contains("ocean freight specialist") || normalizedRole.contains("đường biển"))
                return getOceanFreightSpecialistPrompt();
            if (normalizedRole.contains("air freight specialist") || normalizedRole.contains("đường hàng không"))
                return getAirFreightSpecialistPrompt();
            if (normalizedRole.contains("road freight coordinator") || normalizedRole.contains("đường bộ"))
                return getRoadFreightCoordinatorPrompt();
            if (normalizedRole.contains("customs clearance staff") || normalizedRole.contains("thông quan"))
                return getCustomsClearanceStaffPrompt();
            if (normalizedRole.contains("import export executive") || normalizedRole.contains("xnk"))
                return getImportExportExecutivePrompt();
            if (normalizedRole.contains("shipping documentation officer")
                    || normalizedRole.contains("tài liệu vận tải"))
                return getShippingDocumentationOfficerPrompt();
            if (normalizedRole.contains("vessel planner") || normalizedRole.contains("hãng tàu"))
                return getVesselPlannerPrompt();
        }

        // Supply Chain Management
        boolean isSupplyChain = normalizedIndustry.contains("supply chain")
                || normalizedIndustry.contains("chuỗi cung ứng") ||
        ...
        if (isSupplyChain) {
            if (normalizedRole.contains("supply chain analyst") || normalizedRole.contains("phân tích chuỗi cung ứng"))
                return getSupplyChainAnalystPrompt();
            if (normalizedRole.contains("supply chain manager") || normalizedRole.contains("quản lý chuỗi cung ứng"))
                return getSupplyChainManagerPrompt();
            if (normalizedRole.contains("demand planner") || normalizedRole.contains("hoạch định nhu cầu"))
                return getDemandPlannerPrompt();
            if (normalizedRole.contains("procurement officer") || normalizedRole.contains("mua hàng"))
                return getProcurementOfficerPrompt();
            if (normalizedRole.contains("vendor management specialist")
                    || normalizedRole.contains("quản lý nhà cung cấp"))
                return getVendorManagementSpecialistPrompt();
            if (normalizedRole.contains("order management specialist") || normalizedRole.contains("quản lý đơn hàng"))
                return getOrderManagementSpecialistPrompt();
            if (normalizedRole.contains("production planner") || normalizedRole.contains("kế hoạch sản xuất"))
                return getProductionPlannerPrompt();
        }

        // International Business – Trade
        boolean isInternationalTrade = normalizedIndustry.contains("international business")
                || normalizedIndustry.contains("kinh doanh quốc tế") ||
                ...
                ||
                normalizedIndustry.contains("ecommerce fulfillment")
                || normalizedIndustry.contains("hoàn thành đơn TMĐT");

        if (isInternationalTrade) {
            if (normalizedRole.contains("international sales executive") || normalizedRole.contains("bán hàng quốc tế"))
                return getInternationalSalesExecutivePrompt();
            if (normalizedRole.contains("trade compliance specialist")
                    || normalizedRole.contains("tuân thủ thương mại"))
                return getTradeComplianceSpecialistPrompt();
            if (normalizedRole.contains("global sourcing specialist") || normalizedRole.contains("mua hàng toàn cầu"))
                return getGlobalSourcingSpecialistPrompt();
            if (normalizedRole.contains("international business development")
                    || normalizedRole.contains("phát triển kinh doanh quốc tế"))
                return getInternationalBusinessDevelopmentPrompt();
            if (normalizedRole.contains("foreign trade analyst")
                    || normalizedRole.contains("phân tích thương mại nước ngoài"))
                return getForeignTradeAnalystPrompt();
            if (normalizedRole.contains("commercial invoice specialist")
                    || normalizedRole.contains("hóa đơn thương mại"))
                return getCommercialInvoiceSpecialistPrompt();
            if (normalizedRole.contains("ecommerce fulfillment specialist")
                    || normalizedRole.contains("hoàn thành đơn TMĐT"))
                return getEcommerceFulfillmentSpecialistPrompt();
        }

...
public class InvoiceServiceImpl implements InvoiceService {
    ...
    /**
     * Generate PDF invoice for PaymentTransaction (Premium, Coin Purchase via
     * PayOS)
     */
    ...
    public byte[] generatePaymentInvoice(PaymentTransaction payment, String role) {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            if ("ADMIN".equals(roleUpper)) {
                ...
                addInfoRow(adminTable, "Mã cổng thanh toán:",
                        payment.getReferenceId() != null ? payment.getReferenceId() : "-", labelFont, valueFont);
                addInfoRow(adminTable, "User ID:",
                        payment.getUser() != null ? String.valueOf(payment.getUser().getId()) : "-", labelFont,
                        valueFont);
    ...
    public byte[] generateWalletTransactionInvoice(WalletTransaction transaction, String role) {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            String typeDisplay = transaction.getTransactionType() != null
                    ? transaction.getTransactionType().getDisplayName()
                    : "Giao dịch ví";
            addTableRow(table, typeDisplay, transaction.getDescription() != null ? transaction.getDescription() : "-",
                    amount);
            document.add(table);
            ...
            log.error("Error generating PDF invoice for wallet transaction {}: {}", transaction.getTransactionId(),
                    e.getMessage());
            throw new RuntimeException("Failed to generate invoice PDF", e);
        }
    }
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            if ("ADMIN".equals(roleUpper) || "MENTOR".equals(roleUpper)) {
                ...
                try {
                    ...
                    var payouts = walletTransactionRepository.findByReferenceTypeAndReferenceId("BOOKING_PAYOUT",
                            refId);
                    if (payouts != null && !payouts.isEmpty()) {
                        for (var tx : payouts) {
                            if (tx.getCurrencyType() == WalletTransaction.CurrencyType.CASH
                                    && tx.getCashAmount() != null) {
